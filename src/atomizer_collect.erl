...
-spec collect(pid(), [file:filename()]) -> ok | {error, term()}.
collect(Pid, Paths) ->
...
-spec loop(pid(), non_neg_integer(), sets:set(source()), queue:queue(source())) -> ok | {error, term()}.
loop(Pid, NrParsed, Pool, Queue) ->
    case {sets:size(Pool), queue:len(Queue)} of
        ...
        {NrTakenDescriptors, QueueSize} when NrTakenDescriptors < ?OPEN_FILE_LIMIT, QueueSize > 0 ->
            ...
            spawn_link(atomizer_parse, parse_source, [self(), Source]),
